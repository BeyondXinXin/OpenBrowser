...
void QUIMenu::Menu_File() {
    ...
    connect(Act_file_open, SIGNAL(triggered()),
            I_MainWindow, SIGNAL(SignalOpenFileOut()));
...
// -------------------槽函数-------------------------------------------
void QUIMenu::Act_edit_screenshot() { //截屏
    ScreenWidget::Instance()->showFullScreen();
}
void QUIMenu::qappclose() { // 退出
    qApp->quit();
}
void QUIMenu::File_open() {	// 打开图片
...
void QUIMenu::File_save() {		// 保存文件
}
void QUIMenu::File_open_database() {		// 打开配置文件
    QString path = QFileDialog::getOpenFileName(this,
                   tr("选择图像"), ".",
                   tr("Images(*.ini)"));
    if (path.size() == 0) {
...
void QUIMenu::qappfullScreen() { //全屏
    I_MainWindow->showFullScreen();
}
void QUIMenu::qappmaxScreen() { //退出全屏
    I_MainWindow->showNormal();
}

void QUIMenu::qappNet() { //网络
...
// -------------------帮助-------------------------------------------
void QUIMenu::Help_info() {
}
// -------------------关于-------------------------------------------
void QUIMenu::About_info() {
}
