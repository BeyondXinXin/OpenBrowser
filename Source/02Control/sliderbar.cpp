...
void SliderBar::drawBar(QPainter *painter) {
    ...
    double offset = static_cast<double>(lineWidth / 2);

    if (barStyle == BarStyle_Line_Top) {
        painter->drawLine(
            static_cast<int>(barRect.left()),
            static_cast<int>(barRect.top() + offset),
            static_cast<int>(barRect.right()),
            static_cast<int>(barRect.top() + offset));
    } else if (barStyle == BarStyle_Line_Right) {
        painter->drawLine(
            static_cast<int>(barRect.right() - offset),
            static_cast<int>(barRect.top()),
            static_cast<int>(barRect.right() - offset),
            static_cast<int>(barRect.bottom()));
    } else if (barStyle == BarStyle_Line_Bottom) {
        painter->drawLine(
            static_cast<int>(barRect.left()),
            static_cast<int>(barRect.bottom() - offset),
            static_cast<int>(barRect.right()),
            static_cast<int>(barRect.bottom() - offset));
    } else if (barStyle == BarStyle_Line_Left) {
        painter->drawLine(
            static_cast<int>(barRect.left() + offset),
            static_cast<int>(barRect.top()),
            static_cast<int>(barRect.left() + offset),
            static_cast<int>(barRect.bottom()));
    }
...
void SliderBar::moveTo(const QPointF &point) {
    ...
    for (int i = 0; i < count; i++) {
        if (listItem.at(i).second.contains(point)) {
            ...
            int distance = static_cast<int>(targetLen - barLen);
