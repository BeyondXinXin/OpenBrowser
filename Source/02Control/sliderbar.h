...
class SliderBar : public QWidget {
  ...
  public Q_SLOTS:
    ...
    void setBarStyle(BarStyle barStyle);//设置选中元素样式
    ...
    void moveNext();//往后移动条目
    void moveTo(int index);  //移动到指定索引条目
    void moveTo(const QString &item); //移动到指定文字条目
    void moveTo(const QPointF &point);//移动到指定坐标位置条目
