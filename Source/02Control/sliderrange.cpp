...
void SliderRange::mouseMoveEvent(QMouseEvent *e) {
    ...
    if (leftPressed) {
        if (e->pos().x() >= rect().x()) {
            ...
            double increment = static_cast< double>(width) / (maxValue - minValue);
            int value = static_cast< int >(e->pos().x() / increment);
...
void SliderRange::drawSliderBg(QPainter *painter) {
    ...
    int penWidth = static_cast<int>
                   (height * (static_cast< double>(sliderBgPercent) / 10));
...
void SliderRange::drawSliderLine(QPainter *painter) {
    ...
    int width = this->width();
    int height = this->height();
    sliderLen = static_cast<int>
                (height * (static_cast< double>(sliderPercent) / 10));
    ...
    //计算左右滑块区域
    leftSliderRect = QRect(static_cast<int>(leftValue * increment), initY, sliderLen, height);
    rightSliderRect = QRect(static_cast<int>(rightValue * increment), initY, sliderLen, height);
    ...
    painter->drawRect(leftSliderRect.x(), (height - penWidth) / 2,
                      rightSliderRect.x() - leftSliderRect.x(), penWidth);
    ...
    //绘制左边值指示器
    painter->drawLine(leftSliderRect.x(), sliderLen,
                      leftSliderRect.x(), height - sliderLen);

    //绘制右边值指示器
    painter->drawLine(rightSliderRect.x(), sliderLen,
                      rightSliderRect.x(), height - sliderLen);
...
void SliderRange::drawSliderCircle(QPainter *painter) {
    ...
    int width = this->width();
    int height = this->height();
    sliderLen =  static_cast<int>(height * (static_cast< double>(sliderPercent) / 10));
    ...
    //计算左右滑块区域
    leftSliderRect = QRect(static_cast<int>(leftValue * increment - sliderLen), initY, side, side);
    rightSliderRect = QRect(static_cast<int>(rightValue * increment - sliderLen), initY, side, side);
    ...
    painter->setBrush(usedColor);
    painter->drawRect(leftSliderRect.x() + sliderLen, (height - penWidth) / 2,
                      rightSliderRect.x() - leftSliderRect.x(), penWidth);

    //绘制左边值指示器
    painter->setBrush(borderColor);
    painter->drawEllipse(leftSliderRect.x() - borderWidth,
                         leftSliderRect.y() - borderWidth, borderSide, borderSide);
    painter->setBrush(sliderColor);
    painter->drawEllipse(leftSliderRect);

    //绘制右边值指示器
    painter->setBrush(borderColor);
    painter->drawEllipse(rightSliderRect.x() - borderWidth,
                         rightSliderRect.y() - borderWidth, borderSide, borderSide);
    painter->setBrush(sliderColor);
