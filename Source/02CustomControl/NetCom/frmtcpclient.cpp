...
void frmTcpClient::initConfig() {
    ui->ckHexSend->setChecked(SetUpIni::HexSendTcpClient);
    connect(ui->ckHexSend, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));
    ui->ckHexReceive->setChecked(SetUpIni::HexReceiveTcpClient);
    connect(ui->ckHexReceive, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));
    ui->ckAscii->setChecked(SetUpIni::AsciiTcpClient);
    connect(ui->ckAscii, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));
    ui->ckDebug->setChecked(SetUpIni::DebugTcpClient);
    connect(ui->ckDebug, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));
    ui->ckAutoSend->setChecked(SetUpIni::AutoSendTcpClient);
    connect(ui->ckAutoSend, SIGNAL(stateChanged(int)), this, SLOT(saveConfig()));
    ui->cboxInterval->setCurrentIndex(ui->cboxInterval->findText(QString::number(SetUpIni::IntervalTcpClient)));
    connect(ui->cboxInterval, SIGNAL(currentIndexChanged(int)), this, SLOT(saveConfig()));
    ui->txtServerIP->setText(SetUpIni::TcpServerIP);
    connect(ui->txtServerIP, SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));
    ui->txtServerPort->setText(QString::number(SetUpIni::TcpServerPort));
    connect(ui->txtServerPort, SIGNAL(textChanged(QString)), this, SLOT(saveConfig()));
    timer->setInterval(SetUpIni::IntervalTcpClient);
    SetUpIni::AutoSendTcpClient ? timer->start() : timer->stop();
}

void frmTcpClient::saveConfig() {
    SetUpIni::HexSendTcpClient = ui->ckHexSend->isChecked();
    SetUpIni::HexReceiveTcpClient = ui->ckHexReceive->isChecked();
    SetUpIni::AsciiTcpClient = ui->ckAscii->isChecked();
    SetUpIni::DebugTcpClient = ui->ckDebug->isChecked();
    SetUpIni::AutoSendTcpClient = ui->ckAutoSend->isChecked();
    SetUpIni::IntervalTcpClient = ui->cboxInterval->currentText().toInt();
    SetUpIni::TcpServerIP = ui->txtServerIP->text().trimmed();
    SetUpIni::TcpServerPort = ui->txtServerPort->text().trimmed().toInt();
...
void frmTcpClient::on_btnConnect_clicked() {
    if (ui->btnConnect->text() == "连接") {
        tcpSocket->abort();
        tcpSocket->connectToHost(SetUpIni::TcpServerIP, SetUpIni::TcpServerPort);
    } else {
        tcpSocket->abort();
    }
}
