...
void ProgressBarColor::drawBar(QPainter *painter) {
    ...
    //绘制进度条背景
    QRectF barBgRect(QPointF(space, space),
                     QPointF(width() - space * 2 - textWidth, height() - space));
    painter->setPen(Qt::NoPen);
    painter->setBrush(barBgColor);
    ...
    QRectF barRect(QPointF(space, space),
                   QPointF(space + increment * (value - minValue),
                           height() - space));
