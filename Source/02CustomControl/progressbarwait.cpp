...
void ProgressBarWait::drawArc(QPainter *painter) {
    ...
    QPainterPath pathBig1(QPointF(centerX +
                                  radius * cos(degreesToRadians(currentangle)),
                                  centerY -
                                  radius * sin(degreesToRadians(currentangle))));
    pathBig1.arcTo(centerX - radius, centerY - radius, radius * 2,
                   radius * 2, currentangle, 180);
    pathBig1.arcTo(centerX + radiusBig * cos(degreesToRadians(currentangle + 180)) -
                   radiusBig,
                   centerY - radiusBig * sin(degreesToRadians(currentangle + 180)) -
                   radiusBig,
                   radiusBig * 2, radiusBig * 2, currentangle + 180, 180);
    pathBig1.arcTo(centerX + radiusBig * cos(degreesToRadians(currentangle)) -
                   radiusBig,
                   centerY - radiusBig * sin(degreesToRadians(currentangle)) -
                   radiusBig,
                   radiusBig * 2, radiusBig * 2, currentangle + 180, -180
                  );
    ...
    QPainterPath pathBig2(QPointF(centerX +
                                  radius * cos(degreesToRadians(currentangle)),
                                  centerY -
                                  radius * sin(degreesToRadians(currentangle))));
    pathBig2.arcTo(centerX - radius, centerY - radius,
                   radius * 2, radius * 2, currentangle, -180);
    pathBig2.arcTo(centerX + radiusBig * cos(degreesToRadians(currentangle + 180)) -
                   radiusBig,
                   centerY - radiusBig * sin(degreesToRadians(currentangle + 180)) -
                   radiusBig,
                   radiusBig * 2, radiusBig * 2, currentangle + 180, 180);
    pathBig2.arcTo(centerX + radiusBig * cos(degreesToRadians(currentangle)) -
                   radiusBig,
                   centerY - radiusBig * sin(degreesToRadians(currentangle)) -
                   radiusBig,
                   radiusBig * 2, radiusBig * 2, currentangle + 180, -180
    ...
    pathSmall1.addEllipse(centerX +
                          radiusBig * cos(degreesToRadians(currentangle)) -
                          radiusSmall,
                          centerY - radiusBig *
    ...
    pathSmall2.addEllipse(centerX + radiusBig *
                          cos(degreesToRadians(180 + currentangle)) - radiusSmall,
                          centerY - radiusBig *
                          sin(degreesToRadians(180 + currentangle)) - radiusSmall,
                          radiusSmall * 2, radiusSmall * 2);
...
void ProgressBarWait::drawRoundCircle(QPainter *painter) {
    ...
    for (int i = 0; i < maxValue; i++) {
        ...
        int value = static_cast<int>(i * alpha * 255);
        value = value < 30 ? 30 : value;

        foreground.setAlpha(value);
        painter->setBrush(foreground);
        painter->drawEllipse(static_cast<int>(initX - minRadius),
                             static_cast<int>(initY - minRadius),
...
void ProgressBarWait::drawPie(QPainter *painter) {
    ...
    painter->drawPie(-radius, -radius, radius * 2,
                     radius * 2, startAngle, spanAngle);
    painter->restore();
}
...
void ProgressBarWait::drawLine(QPainter *painter) {
    ...
    for (int i = 0; i <= maxValue; i++) {
        int value = static_cast<int>((currentValue - i) * alpha * 255);
        if (value < 0) {
            value = value + 255;
        }
        value = value < 30 ? 30 : value;
...
void ProgressBarWait::drawText(QPainter *painter) {
    ...
    painter->drawText(rect, Qt::AlignCenter,
                      QString("%1%").arg(currentValue * (100 / maxValue)));
    painter->restore();
}
