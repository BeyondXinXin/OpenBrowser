...
class ProgressBarWait : public QWidget {
  ...
  public:
    ...
    ProgressBarWait(QWidget *parent = nullptr);
    ~ProgressBarWait();
  ...
  public Q_SLOTS:
    void setClockWise(bool clockWise);//设置顺时针逆时针旋转
    void setShowPercent(bool showPercent);//设置是否显示百分比
    void setCurrentValue(int currentValue);//设置当前值
    void setMaxValue(int maxValue);//设置最大值
    void setInterval(int interval);//设置旋转速度间隔
    void setBarStyle(BarStyle barStyle);//设置样式
    void setBackground(const QColor &background);//设置前景色
    void setForeground(const QColor &foreground);//设置前景色
};
